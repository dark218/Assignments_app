...
export class AssignmentInteractionService {
  ...
  getAssignment() {
    this.http.get<{message: string, assignments: any}>('http://localhost:3000/api/assignment')
  ...
  assignmentData.append("remarque", remarque);
    ...
    this.http.post<{message: string, assignment: Assignment}>('http://localhost:3000/api/assignment',assignmentData)
  ...
  addAssignments(assignment: Assignment): Observable<any> {
    ...
    return this.http.post<{message: string, assignment: Assignment}>('http://localhost:3000/api/assignment', assignment);
  }
  ...
  updateAssignment(id: string ,nomdevoir: string, nomeleve: string, datederendu: string, nommatiere: string,nomprof: string,avatar1: File | string, note:string,remarque:string){
    ...
    this.http
             .put('http://localhost:3000/api/assignment/' + id , assignmentData)
             .subscribe(response => {
...
bd.forEach(a => {
  ...
  deleteAssignment(assignmentId: string) {
    this.http.delete('http://localhost:3000/api/assignment/' + assignmentId)
      .subscribe(() => {
