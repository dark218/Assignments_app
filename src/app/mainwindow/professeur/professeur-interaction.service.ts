...
export class ProfesseurInteractionService {
  ...
  getProfesseur() {
    this.http.get<{message: string, professeurs: any}>('http://localhost:3000/api/professeur')
  ...
  getProfesseurs(id: string){
    ...
    ('http://localhost:3000/api/professeur/' + id);
  }
  ...
  addProfesseur( nomprof: string, avatar1: File) {
    ...
    this.http.post<{message: string,professeur: Professeur}>('http://localhost:3000/api/professeur',professeurData)
  ...
  updateProfesseur(id: string ,  nommat: string, nomprof: string ,avatar1: File | string){
    ...
    this.http
             .put('http://localhost:3000/api/professeur/' + id , professeurData)
             .subscribe(response => {
  ...
  deleteProfesseur(professeurId: string) {
    this.http.delete('http://localhost:3000/api/professeur/' + professeurId)
      .subscribe(() => {
